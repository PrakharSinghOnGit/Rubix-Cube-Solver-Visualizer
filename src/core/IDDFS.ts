...
export class IDDFS {
  ...
  calculateStartDepth(size: number): number {
    ...
    switch (size) {
      case 2: return 1; // 2x2 can be solved quickly
      case 3: return 3; // 3x3 needs more depth
      default: return 5; // Larger cubes need more depth
    }
  }
  ...
  doIDDFS() {
    ...
    for (let depth = this.startDepth; depth <= this.maxDepth; depth++) {
      ...
      if (dfs(0, depth, null)) {
        console.log(`Solution found at depth ${depth} with ${this.solutionMoves.length} moves`);
        break;
      }
